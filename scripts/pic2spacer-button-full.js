...
let makeResistatRepeat = function(template, wales, space, offset, offsetCntr) {
    ...
    for(let k = 0; k < wales; k++) {
        ...
        let c = template[k] === 'x' ? (k % 2 ? 't' : 'T' ) : (template[k] === 'b' ? r[p] : '.');
        if(c !== '.') {
            if(!firstOp)
                firstOp = k;
            lastOp = k;
...
let makeFillerRepeat = function(template, wales, space, offset, offsetCntr) {
    ...
    for(let k = 0; k < wales; k++) {
        ...
        let c = template[k] === 'x' ? (k % 2 ? 't' : 'T' ) : (template[k] === 'f' || template[k] === 'b' ? r[p] : '.');
        if(c !== '.') {
            if(!firstOp)
                firstOp = k;
            lastOp = k;
    ...
    .then(img => {
        ...
        for(let j = 0; j < courses; j++) {
            ...
            for(let i = 0; i < wales; i++) {
                ...
                {
                    case 0x3f:
                        template += 'f';     //filler area, no Resistat
                        break;
                    case 0x7f:
            ...
            for( let i = 0; i < 2; i++ ) {
                if(y[i] === frontYarn) {
                    ...
                    for( let k = 0; k < template.length; k++ )
                        repeat2nd += template[k] === 'b' ? '2' : '.';

                    ks.newCourse(y[i]);
                    ks.insert(y[i], faceRepeat[i], faceRepeat[i].length);
                } else {
                    ks.newCourse(y[i]);
            ...
            if(densityCntr >= densityFace) {
                for(let i = 0; i < densityFiller; i++, offsetCntr++) {

                    {
                        ...
                        ks.newCourse(yarnNylon, firstOp);
                        ks.insert(yarnNylon, repeat.substring(firstOp, lastOp + 1));
                    }

                    {
                        let {repeat, firstOp, lastOp} = makeResistatRepeat(template, wales, space, offset, offsetCntr);

                        if(firstOp !== undefined) {
                            buttonOngoing = true;
                            ks.newCourse(yarnResistat, firstOp);
                            ks.insert(yarnResistat, repeat.substring(firstOp, lastOp + 1));
                        } else {
                            if(buttonOngoing) {
                                ks.cut(yarnResistat);
                                buttonOngoing = false;
